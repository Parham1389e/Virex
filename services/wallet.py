...
from database.models import LedgerEntry, TopUp, User, Wallet
...
class WalletService:
    ...
    async def apply(self, user_id: int, amount: Decimal, kind: str, key: str, reference: str | None = None):
        ...
        entry = LedgerEntry(wallet_id=wallet.id, amount=amount, balance_after=new_balance, kind=kind, idempotency_key=key, reference=reference)
        self.session.add(entry); await self.session.flush(); return entry
    async def approve_topup(self, topup_id: int, decision_key: str):
        ...
        topup.status = "approved"; topup.decision_key = decision_key
        await self.apply(topup.user_id, topup.amount, "topup", f"topup:{topup.id}", str(topup.id)); return topup
