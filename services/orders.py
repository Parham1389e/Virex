...
class OrderService:
    ...
    async def purchase(self, user_id: int, product_id: int, key: str):
        ...
        await WalletService(self.session).apply(user_id, -Decimal(product.price), "purchase", key, str(product_id)); config.assigned = True
        order = Order(user_id=user_id, product_id=product_id, amount=product.price, status="paid", payment_key=key, config_id=config.id); self.session.add(order); await self.session.flush(); return order
    async def refund(self, order_id: int, key: str):
        ...
        await WalletService(self.session).apply(order.user_id, Decimal(order.amount), "refund", key, str(order.id)); order.status = "refunded"; return order
    async def mark_delivered(self, order_id: int):
        order = (await self.session.execute(select(Order).where(Order.id == order_id).with_for_update())).scalar_one()
        if order.delivered_at is None: order.delivered_at = datetime.now(timezone.utc)
        return order
