...
class OrderService:
    ...
    async def purchase(self, user_id: int, product_id: int, key: str) -> Order:
        existing = await self.session.scalar(select(Order).where(Order.payment_key == key))
        ...
        if config is None: raise ValueError("no config available")
        await WalletService(self.session).apply(user_id, -Decimal(product.price), "purchase", key, str(product_id))
        config.assigned = True
        order = Order(user_id=user_id, product_id=product_id, amount=product.price, payment_key=key, config_id=config.id)
        self.session.add(order); await self.session.flush(); return order

    async def refund(self, order_id: int, key: str) -> Order:
    ...
    async def mark_delivered(self, order_id: int) -> Order:
        order = (await self.session.execute(select(Order).where(Order.id == order_id).with_for_update())).scalar_one()
        if order.delivery_status == "sent": return order
        order.delivery_status = "sent"; order.delivered_at = datetime.now(timezone.utc); return order
